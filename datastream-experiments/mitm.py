    ...
    def handle(self):
        ...
        while True:
            ...
            try:
                ...
                return
            log('<-' if curDir == 'in' else '->', f'({len(dat)}) byte', f'{dat!r}' if len(dat) < 100 else 'packet too big', f'Source Port: {self.request.getsockname()[1]}', f'Target Port: {outConn.getsockname()[1]}', f'Timestamp: {time.time()}')
